...
const Horarios = (() => {
  ...
  function htmlDisciplina(disciplina) {
    ...
    const slotsHtml = entradas.length
      ? entradas.map((e, i) => htmlSlot(disciplina, e, i)).join("")
    ...
    for (let i = 0; i < FAIXAS.length; i++) {
      ...
      DIAS.forEach((dia) => {
        ...
        if (ocupantes.length === 1) {
          ...
          // conflito: mais de uma disciplina na mesma faixa/dia
          // calcula quantas faixas consecutivas têm exatamente o mesmo conjunto de disciplinas em conflito
          let span = 1;
          for (let j = i + 1; j < FAIXAS.length; j++) {
            const prox = grade[dia][j];
            const mesmoConjunto =
